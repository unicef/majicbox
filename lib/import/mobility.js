...
/**
 * Return row whether row is valid
 *
 * @param{obj} obj - row from mobility file
 * @return{bool}.
 */
function check_row_is_valid(obj) {
  var missing = Object.keys(obj).find(
    function(e) {return !obj[e];}
  );
  if (missing) {
    console.log(missing, 'missing in', obj);
    return false;
  } else {
    return true;
  }
  ...
  .then(function(files) {
    files.forEach(function(file) {
      ...
      result = result.then(function() {
        return import_file(dir + '/' + file, country_code, verbose);
      });
    });
    return result;
  });
