    ...
    var csvStream = csv()
        .on('data', function(data) {
          ...
          if ((count > 0) && (data.length > 3)) {
            ...
            var mobility = new Mobility(o);

            mobility.save(function(err) {
              if (err) {
                console.log(err);
              } else {
...
function get_mobility_files(dir) {
  return new Promise(function(resolve, reject) {
    fs.readdir(dir, function(err, list) {
      if (err) { console.log(err, 'xxxx'); return reject(err);}
...
// Currently you need to copy csv files to ./data/daily_mobility to run this
// Change later.
// node import.js daily_migrations br br
