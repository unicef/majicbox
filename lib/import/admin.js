...
function save_topojson(country_code, feature_collection) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    RegionTopojson.find({country_code: country_code}).remove(function(err) {
      if (err) {
        return reject(err);
      }
      RegionTopojson.insertMany([{
 ...
 * @return{Promise} Fulfilled when documents are saved.
 */
function save_features(country_code, feature_collection) {
  var admins = [];
...
function import_admins(country_code, geojson, population, verbose) {
  ...
  .then(function(feature_collection) {
    return save_features(country_code, feature_collection);
  })
  .then(function(feature_collection) {
