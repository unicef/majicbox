 ...
 *
 * @param{string} country_code - 3166-1 alpha-2 country code.
 * @param{string} population_file - name of population file.
 * @param{string} admin_geojson_file - name of geojson file.
 * @param{string} verbose - Option to display debug
 * @return{Promise} Fulfilled when documents are saved.
 */
function import_admins(country_code, admin_geojson_file, population_file, verbose) {
  var worldpop_file = './data/population/' + population_file;
  return Promise.all([
    read_jsonfile(admin_geojson_file, verbose),
    world_pop(worldpop_file)
  ])
  ...
  .then(function(feature_collection) {
    return save_topojson(country_code, feature_collection, admin_geojson_file, verbose);
  });
}
...
function main() {
  ...
  mongoose.connect(mongodb, function(err) {
    if (err) { throw err; }
    import_admins(args.country_code, args.file, args.population, args.verbose)
    .then(function() {
      return process.exit();
