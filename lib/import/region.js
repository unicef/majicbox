    ...
    var promise = new Promise(function(resolve) {
      region.save(function(err) {
        if (err) {
          ...
          console.log(count++, feature.name, ' Region saved!');
        }
        resolve();
      });
    });
 ...
 * @return{Promise} Fulfilled when documents are saved.
 */
function import_regions(country_code, file, path) {
  path = path + country_code + '/' + file + '.json';
...
function main() {
  ...
  parser.addArgument(
    ['-p', '--path'],
    {
      help: 'Path to file to be imported.'
    }
  );
  ...
  mongoose.connect(mongodb, function(err) {
    ...
    var path = './data/geojson/';
    import_regions(args.country_code, args.file, path)
    .then(function() {
      return process.exit();
    });
  });
