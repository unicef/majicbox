...
var add_days = function(date, num_days) {
 ...
 */
function add_fake_mobility(country_code, date, num_days, min_count, jitter_range, dry_run) {
  jitter_range = jitter_range || [-0.05, 0.05];
  ...
  var dates = _.range(num_days).map(function(num_next_days) {
    ...
    return add_days(date, num_next_days + 1);
  });

  // Promise of an array of mobility objects.
  var base_mobility_promise = new Promise(function(res, rej) {
  ...
  return base_mobility_promise.then(function(mobilities) {
    ...
    return Promise.all(dates.map(function(date) {
      ...
      return new Promise(function(res, rej) {
        Mobility.insertMany(_.clone(mobilities), function(err) {
          if (err) { return rej(err); }
...
function main() {
  ...
  mongoose.connect(config.database, function(err) {
    if (err) { throw err; }
    add_fake_mobility(args.country_code, date, num_days, min_count)
      .then(process.exit)
